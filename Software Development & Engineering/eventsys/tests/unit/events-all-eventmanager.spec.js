...
describe('Events-All (Logged in as Event-Manager)', () => {
  let wrapper;
  let store;
  ...
  beforeEach(() => {
    ...
    store.state.isEventManager = true;
  ...
  it('should have a create event button', (done) => {
    wrapper.vm.$nextTick(() => {
        expect(wrapper.find("#btnCreateEvent").exists()).to.be.true;
        done();
    });
  })
